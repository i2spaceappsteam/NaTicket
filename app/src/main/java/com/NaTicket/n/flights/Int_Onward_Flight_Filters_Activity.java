...
public class Int_Onward_Flight_Filters_Activity  extends BackActivity implements View.OnClickListener{
    ...
    ArrayList<InternationalFlightsDTO> int_onward_list;
    ...
    public void getdatatoset(){
        for (int i=0;i<int_onward_list.size();i++){
            double d = Double.parseDouble(String.valueOf(int_onward_list.get(i).getFareDetails().getChargeableFares().getActualBaseFare()));
            double Servicecharge = Double.parseDouble(String.valueOf(int_onward_list.get(i).getFareDetails().getChargeableFares().getTax()));
            double Operatercharge = Double.parseDouble(String.valueOf(int_onward_list.get(i).getFareDetails().getNonchargeableFares().getTCharge()));
            double Markepcharge = Double.parseDouble(String.valueOf(int_onward_list.get(i).getFareDetails().getNonchargeableFares().getTMarkup()));
            final double TotalFare=d+Servicecharge+Operatercharge+Markepcharge;
            Amount.add(Util.getprice(TotalFare));
            Airlines.add(int_onward_list.get(i).getIntOnward().getFlightSegments().get(0).getAirLineName());
            Fare_Type.add(int_onward_list.get(i).getIntOnward().getFlightSegments().get(0).getBookingClassFare().getRule());
            Stops.add(Flight_Utils.getLengthforfilters(int_onward_list.get(i).getIntOnward().getFlightSegments().size()));
        }
    ...
    private void SetAirlines_Layout(ArrayList<String> Airlines,LinearLayout Check_box_layout){
        if(Airlines.size()>0){
            ...
            for(int i=0;i<Airlines.size();i++){
                final CheckBox checkBox = new CheckBox(Int_Onward_Flight_Filters_Activity.this);
                FontTypeface fontTypeface=new FontTypeface(Int_Onward_Flight_Filters_Activity.this);
