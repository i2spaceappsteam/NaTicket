...
public class Flight_Ticket_Details extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        Refernce_Number=extras.getString("referencenumber");
    ...
    private void callGetFlightTicketDetails(){
        if(Util.isNetworkAvailable(getApplicationContext())) {
            ...
            Service_Reports.GetFlightTicket_Details(Flight_Ticket_Details.this, Constants.FlightTicketDetails+Refernce_Number+"&type=2");
        }else{
            Util.showMessage(Flight_Ticket_Details.this, Constants.NO_INT_MSG);
        }
    }
    ...
    public void setFlightData(){
        ...
        for (int p=0;p<flight_ticket_details.getOnwardFlightSegments().size();p++){
            ...
            String From_Time= Flight_Utils.getTime(flight_ticket_details.getOnwardFlightSegments().get(p).getDepartureDateTime());
            String To_Time=Flight_Utils.getTime(flight_ticket_details.getOnwardFlightSegments().get(p).getArrivalDateTime());
            ...
            //Bitmap bitmap= Flight_Utils.getImage(this,flight_ticket_details.getOnwardFlightSegments().get(p).getImageFileName());
           // Airline_Image.setImageBitmap(bitmap);

            from.setText(""+From_Place+"");
            to.setText(""+To_Place+"");
            FromDate.setText(Flight_Utils.getDate(flight_ticket_details.getOnwardFlightSegments().get(p).getDepartureDateTime()));
            ToDate.setText(Flight_Utils.getDate(flight_ticket_details.getOnwardFlightSegments().get(p).getArrivalDateTime()));
        ...
        if (flight_ticket_details.getReturnFlightSegments()!=null&&flight_ticket_details.getReturnFlightSegments().size()!=0) {
            for (int p = 0; p < flight_ticket_details.getReturnFlightSegments().size(); p++) {
                ...
                String From_Time = Flight_Utils.getTime(flight_ticket_details.getReturnFlightSegments().get(p).getDepartureDateTime());
                String To_Time = Flight_Utils.getTime(flight_ticket_details.getReturnFlightSegments().get(p).getArrivalDateTime());
...
//                Bitmap bitmap = Flight_Utils.getImage(this, flight_ticket_details.getReturnFlightSegments().get(p).getImageFileName());
//                Airline_Image.setImageBitmap(bitmap);

                from.setText(""+From_Place+"");
                to.setText(""+To_Place+"");
                FromDate.setText(Flight_Utils.getDate(flight_ticket_details.getReturnFlightSegments().get(p).getDepartureDateTime()));
                ToDate.setText(Flight_Utils.getDate(flight_ticket_details.getReturnFlightSegments().get(p).getArrivalDateTime()));
                FromTime.setText(From_Time);
                ToTime.setText(To_Time);
...
//            double Return_Total=ReturnBaseFare+ReturnTax+ReturnTcharge+ReturnTMarkup+ReturnSTax+ReturnConvFee;
        ...
        BookingDate.setText(""+Flight_Utils.getDate(flight_ticket_details.getBookingDate())+"");
    ...
    private void initViews() {
        ...
        Cancel_Ticket_Report.setVisibility(View.GONE);
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Intent ip=new Intent(Flight_Ticket_Details.this,Flights_Search_Activity.class);
