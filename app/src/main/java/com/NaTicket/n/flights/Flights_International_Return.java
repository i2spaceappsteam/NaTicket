...
public class Flights_International_Return extends BackActivity {
    ...
    International_Return_Adapter flights_adapter;
    ...
    private void callFlightsData(){
        ...
        if(Util.isNetworkAvailable(getApplicationContext())) {
            ...
            ServiceClasses.getAvaliableFlights(Flights_International_Return.this,url);
    ...
    private void setFlightsData(ArrayList<InternationalFlightsDTO> availableFlights) {
        flights_adapter   = new International_Return_Adapter(this,availableFlights);
    ...
    private void sortfilter(){
        sort.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                BottomSheetDialog dialog=new BottomSheetDialog(Flights_International_Return.this);
    ...
    private void callTaxDetails(){
        ...
        if (Util.isNetworkAvailable(getApplicationContext())) {
            ServiceClasses.getTaxDetails(Flights_International_Return.this,Constants.GetTaxDetails,prepayload());
        } else {
            Util.showMessage(this, Constants.NO_INT_MSG);
        }
    ...
    private String prepayload() {
        ...
        try {
            for(int i=0;i<SelFlight.getIntOnward().getFlightSegments().size();i++) {
                onward_array.put(new JSONObject(gson.toJson(SelFlight.getIntOnward().getFlightSegments().get(i))));
            }

            for(int i=0;i<SelFlight.getIntReturn().getFlightSegments().size();i++) {
                ret_array.put(new JSONObject(gson.toJson(SelFlight.getIntReturn().getFlightSegments().get(i))));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            bcparam.put("Provider", SelFlight.getProvider());
            ...
            bcparam.put("ImagePath", SelFlight.getIntOnward().getFlightSegments().get(0).getImageFileName());
            bcparam.put("ImagePathRet", SelFlight.getIntReturn().getFlightSegments().get(0).getImageFileName());
            bcparam.put("Rule", SelFlight.getIntOnward().getFlightSegments().get(0).getBookingClassFare().getRule());
            bcparam.put("key", SelFlight.getOriginDestinationoptionId().getKey());
            bcparam.put("RuleRet", SelFlight.getIntReturn().getFlightSegments().get(0).getBookingClassFare().getRule());
            bcparam.put("keyRet",SelFlight.getOriginDestinationoptionId().getKey());
            bcparam.put("FlightId", "");
            bcparam.put("FlightIdRet", "");
            bcparam.put("OnwardFlightSegments",onward_array);
            bcparam.put("ReturnFlightSegments", ret_array);
            bcparam.put("FareDetails", new JSONObject(gson.toJson(SelFlight.getFareDetails())));
            bcparam.put("BookingDate", "0001-01-01T00:00:00");
            bcparam.put("PromoCode", "");
            bcparam.put("PromoCodeAmount", "0");
            bcparam.put("PostMarkup", "");
            bcparam.put("OcTax", SelFlight.getIntOnward().getFlightSegments().get(0).getOcTax());
            bcparam.put("ActualBaseFare", SelFlight.getFareDetails().getChargeableFares().getActualBaseFare());
            bcparam.put("Tax", SelFlight.getFareDetails().getChargeableFares().getTax());
            bcparam.put("STax", SelFlight.getFareDetails().getChargeableFares().getSTax());
            bcparam.put("SCharge", SelFlight.getFareDetails().getChargeableFares().getSCharge());
            bcparam.put("TDiscount", SelFlight.getFareDetails().getChargeableFares().getTDiscount());
            bcparam.put("TPartnerCommission", SelFlight.getFareDetails().getChargeableFares().getTPartnerCommission());
            bcparam.put("TCharge", SelFlight.getFareDetails().getNonchargeableFares().getTCharge());
            bcparam.put("TMarkup", SelFlight.getFareDetails().getNonchargeableFares().getTMarkup());
            bcparam.put("TSdiscount", SelFlight.getFareDetails().getNonchargeableFares().getTSdiscount());
            bcparam.put("TransactionId", "");
            bcparam.put("ActualBaseFareRet", "0.0");
            bcparam.put("TaxRet", "0.0");
            bcparam.put("STaxRet", "0.0");
            bcparam.put("SChargeRet", "0.0");
            bcparam.put("TDiscountRet", "0.0");
            bcparam.put("TSDiscountRet", "0.0");
            bcparam.put("TPartnerCommissionRet", "0.0");
            bcparam.put("TChargeRet", "0.0");
            bcparam.put("TMarkupRet", "0.0");
    ...
    public void postTaxDetailsResponse(String response) {
        ...
        if (response!=null){
            ...
            if (Util.getTaxResponseCode(response)==14){
                ...
                final double TotalFare = d + Servicecharge + Operatercharge + Markepcharge;
                SelFlight.setFareDetails(fareDetailsDTO);
